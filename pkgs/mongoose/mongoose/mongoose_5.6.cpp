...
static int call_user(struct connection *conn, int ev) {
#ifdef NS_ENABLE_SSL
  ...
  while ((rewrites = next_option(rewrites, &a, &b)) != NULL) {
    if ((match_code = atoi(a.ptr)) > 0 && match_code == code) {
      ...
      mg_printf(c, "HTTP/1.1 %d Moved\r\n"
                "Location: %.*s?code=%d&orig_uri=%s&query=%s\r\n\r\n",
...
static void prepare_cgi_environment(struct connection *conn,
  ...
  if (ri->query != NULL)
    addenv(blk, "QUERY_STRING=%s", ri->query);
...
static size_t parse_http_message(char *buf, size_t len,
  ...
  ri->request_method = ri->uri = ri->http_version = ri->query = NULL;
...
xreal_printf("parse_http_message len = ~0\n");
    ...
    if ((ri->query = strchr(ri->uri, '?')) != NULL) {
      *(char *) ri->query++ = '\0';
    }
    n = (int) strlen(ri->uri);
...
static void ping_idle_websocket_connection(struct connection *conn, time_t t) {
  if (t - conn->ns_conn->last_io_time > MONGOOSE_USE_WEBSOCKET_PING_INTERVAL) {
    mg_websocket_write(&conn->mg_conn, WEBSOCKET_OP_PING, "", 0);
  }
}
#else
...
static int __cdecl compare_dir_entries(const void *p1, const void *p2) {
  ...
  const char *qs = a->conn->mg_conn.query ?
    a->conn->mg_conn.query : "na";
  int cmp_result = 0;
...
static void send_directory_listing(struct connection *conn, const char *dir) {
  ...
  int i, num_entries, sort_direction = conn->mg_conn.query != NULL &&
    conn->mg_conn.query[1] == 'd' ? 'a' : 'd';
...
static void close_local_endpoint(struct connection *conn) {
  ...
  c->request_method = c->uri = c->http_version = c->query = NULL;
...
int mg_get_var(const struct mg_connection *conn, const char *name,
               char *dst, size_t dst_len) {
  int len = get_var(conn->query, conn->query == NULL ? 0 :
                    strlen(conn->query), name, dst, dst_len);
  if (len < 0) {
