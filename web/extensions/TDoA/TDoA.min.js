...
/* web/extensions/TDoA/TDoA.min.js (web/extensions/TDoA/TDoA.js = Sun May 11 10:20:15 2025) */
...
if(h.mkr){h.have_event_listeners=[];h.id_snr=h.id;if(h.snr>0)h.id_snr+=' '+h.snr;if(h.selected){if(tdoa.leaflet)
tdoa_style_marker(h.mkr,h.idx,h.id_snr,'host',tdoa.kiwi_map);else
h.mkr.setMap(tdoa.kiwi_map);}else{if(show_hosts){if(tdoa.leaflet)tdoa_style_marker(h.mkr,h.idx,h.id_snr,'host');tdoa.cur_host_markers.push(h.mkr);}}}}
...
tdoa_rebuild_hosts();if(f.mkr){tdoa_change_marker_style(f.mkr,'red','white','cl-tdoa-gmap-selected-label');}
...
if(!tdoa.old_algorithm)s+=",'new',true";if(tdoa.kml)s+=",'kml',1";s+=')';if(tdoa.rerun)s+='&rerun='+tdoa.response.key;if(tdoa.devl)s+='&devl=1';tdoa.last_menu_select=undefined;tdoa.response={};tdoa.response.seq=0;tdoa.response.key=(Date.now()%100000).leadingZeros(5);kiwi_ajax_progress(tdoa.url_base+'php/tdoa.php'+tdoa.a[0]+'&key='+tdoa.response.key+s,function(json){},0,0,'',0);var expire=Date.now()+200000;tdoa.progress_interval=setInterval(function(){if(tdoa.response.key){kiwi_ajax(tdoa.url_files+tdoa.response.key+'/progress.json',function(json){tdoa_protocol_response_cb(json);if(tdoa.response.seq==tdoa.SEQ_DONE){kiwi_clearInterval(tdoa.progress_interval);}});}
...
function tdoa_result_menu_click_cb(path,idx,first){var i,k,s,url;idx=+idx;tdoa.last_menu_select=idx;tdoa_KML_link('');if(tdoa.leaflet){if(tdoa.map_layers){tdoa.map_layers.forEach(function(ml){if(ml)ml.remove();});tdoa.map_layers=[];}
