...
function tdoa_host_marker_click(mkr,ev){var j;var h=mkr.kiwi_mkr_2_ref_or_host;if(ev&&ev.shiftKey){tdoa_preview_click(h,ev);return;}
for(j=0;j<tdoa.tfields;j++){if(w3_get_value('tdoa.id_field-'+j)==h.call)return;}
for(j=0;j<tdoa.tfields;j++){var f=tdoa.field[j];if(f.inuse)continue;var url=h.hp;f.inuse=true;f.good=true;f.use=true;f.mkr=mkr;f.host=h;h.selected=true;w3_set_value('tdoa.id_field-'+j,h.call);w3_set_value('tdoa.url_field-'+j,url);tdoa_set_icon('sample',j,'fa-refresh fa-spin',20,'lightGrey');tdoa_rerun_clear();var s=(dbgUs&&url=='kiwisdr.jks.com:8073')?'www:8073':url;kiwi_ajax('http://'+s+'/status','tdoa_host_click_status_cb',j,5000);break;}}
...
function tdoa_listen_cb(path,val,first,ev){var field_idx=+val;var f=tdoa.field[field_idx];if(f){var host=f.mkr?f.mkr.kiwi_mkr_2_ref_or_host:f.host;if(ev&&ev.shiftKey){tdoa_preview_click(host,ev);}else{tdoa_open_window(host);}}}
...
el.title=rh.title;el.kiwi_mkr_2_ref_or_host=rh;if(!rh.have_event_listeners[i]){rh.have_event_listeners[i]=true;el.addEventListener('click',function(ev){tdoa_marker_click(ev.target,ev);},w3.BUBBLING);el.addEventListener('dblclick',function(ev){tdoa_marker_dblclick(ev.target);},w3.BUBBLING);el.addEventListener('mouseenter',function(ev){if(!rh.selected){w3_color(el,'black','yellow');el.style.zIndex=9001;}
tdoa_clear_unspiderfy_timeout();},w3.BUBBLING);el.addEventListener('mouseleave',function(ev){if(!rh.selected){if(rh.type_host)
w3_color(el,'white',rh.lo?'red':'blue');else
w3_color(el,'black','lime');el.style.zIndex=9000;}
tdoa_set_unspiderfy_timeout();},w3.BUBBLING);}}});});if(map){marker.addTo(map);tdoa.map_layers.push(marker);}}
...
function tdoa_rebuild_hosts(opts){if(!tdoa.hosts)return;tdoa_remove_hosts();if(tdoa.last_result_map_select>tdoa.COMBINED_MAP)return;var show_hosts=(opts==undefined)?tdoa.show_hosts:opts.show;tdoa.cur_host_markers=[];for(var i=0;i<tdoa.hosts.length;i++){var h=tdoa.hosts[i];var v=+h.v;if(tdoa.hosts_sel&&((tdoa.hosts_sel==1&&v>1.646)||(tdoa.hosts_sel==2&&v<1.653))){continue;}
if(h.mkr){h.have_event_listeners=[];h.id_snr=h.id;if(h.snr>0)h.id_snr+=' '+h.snr;if(tdoa.last_result_map_select!=tdoa.TDOA_MAP_NO_HOSTS){if(h.selected){if(tdoa.leaflet){tdoa_style_marker(h.mkr,h.idx,h.id_snr,'host',tdoa.kiwi_map);}else{h.mkr.setMap(tdoa.kiwi_map);}}else{if(show_hosts){if(tdoa.leaflet)tdoa_style_marker(h.mkr,h.idx,h.id_snr,'host');tdoa.cur_host_markers.push(h.mkr);}}}}}
...
ids.push(id);});tdoa.cur_ref_markers=[];var show_refs=(opts==undefined)?tdoa.show_refs:opts.show;if(show_refs){for(var i=tdoa.FIRST_REF;i<tdoa.refs.length-1;i++){var r=tdoa.refs[i];r.have_event_listeners=[];var done=false;ids.forEach(function(id){if(r.r.includes(id)&&!done){if(r.selected){if(tdoa.leaflet)
...
for(j=0;j<i;j++){var h0=hosts[j];if(tdoa.leaflet){var h_ll=L.latLng(h.lat,h.lon);var h0_ll=L.latLng(h0.lat,h0.lon);var spacing=h_ll.distanceTo(h0_ll);if(spacing<2000){if(h0.dither_idx==undefined){h0.dither_idx=1;}
h.dither_idx=h0.dither_idx++;tdoa.hosts_dither.push(h);}}
if(h0.id==h.id){if(h0.dup==undefined)h0.dup=0;h0.dup++;h.id+='/'+h0.dup;}}
h.id_lcase=h.id.toLowerCase();h.mkr=tdoa_place_host_marker(h,tdoa.kiwi_map);}
var lat,lon,zoom,maptype,init_submit;if(isArg(tdoa.params))console.log(tdoa.params);if(tdoa.params){var p=tdoa.params.split(',');tdoa.params=null;for(i=0,len=p.length;i<len;i++){var a=p[i];console.log('TDoA: param <'+a+'>');var r;if(a.includes(':')){if(a.startsWith('lat:')){lat=parseFloat(a.substring(4));}else
...
w3_set_value('tdoa.id_field-'+j,r[0]);var url='';if(isNonEmptyString(r[1]))url=r[1];if(isNonEmptyString(r[2]))url+=':'+r[2];if(isNonEmptyString(url)){tdoa_edit_url_field_cb('tdoa.url_field-'+j,url);tdoa.field[j].inuse=true;}
break;}}}else
...
tdoa_set_icon('sample',field_idx,'fa-refresh fa-spin',20,'lightGrey');tdoa_rerun_clear();var s=(dbgUs&&val=='kiwisdr.jks.com:8073')?'www:8073':val;var hp=val.split(':');f.host={h:hp[0],p:hp[1]};kiwi_ajax('http://'+s+'/status','tdoa_host_click_status_cb',-field_idx,5000);tdoa_rebuild_hosts();}
...
s=fixes_min?('GPS: '+fixes_min+' fixes/min'):'channel available';w3_innerHTML('id-tdoa-sample-status-'+field_idx,s);if(create_mkr){var h=f.host;h.i=h.idx=tdoa.hosts.length;h.id=tdoa_id;h.lat=lat;h.lon=lon;h.snr=snr;h.a=ant;h.n=name;h.u=users;h.um=users_max;h.v=ver;h.tc=tdoa_ch;h.fm=fixes_min;h.title=tdoa_host_title(h);h.id_lcase=h.id.toLowerCase();f.mkr=h.mkr=tdoa_place_host_marker(h,tdoa.kiwi_map);h.selected=true;tdoa.hosts.push(h);}
if(f.host)console.log(f.host);tdoa_rebuild_hosts();if(f.mkr){tdoa_change_marker_style(f.mkr,'red','white','cl-tdoa-gmap-selected-label');}
...
if(!tdoa.old_algorithm)s+=",'new',true";if(tdoa.kml)s+=",'kml',1";s+=')';if(tdoa.rerun)s+='&rerun='+tdoa.response.key;if(tdoa.devl)s+='&devl=1';tdoa.last_result_map_select=tdoa.KIWI_MAP;tdoa.response={};tdoa.response.seq=0;tdoa.response.key=(Date.now()%100000).leadingZeros(5);kiwi_ajax_progress(tdoa.url_base+'php/tdoa.php'+tdoa.a[0]+'&key='+tdoa.response.key+s,function(json){},0,0,'',0);var expire=Date.now()+250000;tdoa.progress_interval=setInterval(function(){if(tdoa.response.key){kiwi_ajax(tdoa.url_files+tdoa.response.key+'/progress.json',function(json){tdoa_protocol_response_cb(json);if(tdoa.response.seq==tdoa.SEQ_DONE){kiwi_clearInterval(tdoa.progress_interval);}});}
...
w3_innerHTML('id-tdoa-results-select',w3_select('w3-text-red','','TDoA results','tdoa.result_select',-1,tdoa.results,'tdoa_result_menu_click_cb'));w3_show('id-tdoa-results-select');w3_show('id-tdoa-results-options');var result_map=tdoa.result_hosts?tdoa.TDOA_MAP_WITH_HOSTS:tdoa.TDOA_MAP_NO_HOSTS;tdoa_result_menu_click_cb('',result_map);w3_select_value('tdoa.result_select',result_map);var which='TDoA';if(tdoa.old_algorithm)which+='-old';else
...
function tdoa_result_menu_click_cb(path,idx,first){var i,k,s,url;idx=+idx;tdoa.last_result_map_select=idx;tdoa_KML_link('');if(tdoa.leaflet){if(tdoa.map_layers){tdoa.map_layers.forEach(function(ml){if(ml)ml.remove();});tdoa.map_layers=[];}
...
tdoa_show_maps({kiwi:false,result:true});url=tdoa.url_files+tdoa.response.key+'/tdoa_data.mat';w3_innerHTML('id-tdoa-download-MAT',w3_link('',url,w3_icon('w3-text-aqua','fa-download',18))+' MAT');var ms,me;if(idx==tdoa.COMBINED_MAP){ms=tdoa.SINGLE_MAPS;me=tdoa.SINGLE_MAPS+npairs;}else{ms=idx;me=ms+1;}
...
parser=new DOMParser();kml=parser.parseFromString(j.text,'text/xml');track=new L.KML(kml);tdoa.result_map.addLayer(track);}else{if(j.polygons&&j.polygons.length){pg=new Array(j.polygons.length);for(i=0;i<j.polygons.length;++i){p=j.polygons[i];poly=[];for(k=0;k<p.length;k++){poly[k]=[p[k].lat,p[k].lng];}
...
pl[i]=L.polyline(poly,{color:j.polyline_colors[i],opacity:1,weight:1,fillOpacity:0});pl[i].addTo(tdoa.result_map);tdoa.map_layers.push(pl[i]);}}}}else{tdoa.heatmap[midx]=new google.maps.GroundOverlay(tdoa.url_files+j.filename,j.imgBounds,{opacity:0.5});tdoa.heatmap[midx].setMap(tdoa.heatmap_visible?tdoa.result_map:null);if(j.polygons&&j.polygons.length){pg=new Array(j.polygons.length);for(i=0;i<j.polygons.length;++i){pg[i]=new google.maps.Polygon({paths:j.polygons[i],strokeColor:j.polygon_colors[i],strokeOpacity:1,strokeWeight:1,fillOpacity:0});pg[i].setMap(tdoa.result_map);}}
if(j.polylines&&j.polylines.length){pl=new Array(j.polylines.length);for(i=0;i<j.polylines.length;++i){pl[i]=new google.maps.Polyline({path:j.polylines[i],strokeColor:j.polyline_colors[i],strokeOpacity:1,strokeWeight:1,fillOpacity:0});pl[i].setMap(tdoa.result_map);}}}},mi);}
...
for(i=0;i<show_mkrs.length;i++){var field_idx=show_mkrs[i];if(field_idx==undefined)continue;var f=tdoa.field[field_idx];if(f.mkr&&f.host){if(idx==tdoa.TDOA_MAP_NO_HOSTS){mkr=f.host.mkr;tdoa_marker_remove(mkr);}else{tdoa_place_host_marker(f.host,tdoa.result_map);}}}}
...
tdoa.result_refs.push(ref);tdoa.result_mkrs=[];tdoa.result_refs.forEach(function(ref,idx){mkr=tdoa_place_ref_marker(idx+1000,tdoa.result_map,ref);tdoa.result_mkrs.push(mkr);});}}else{tdoa_show_maps({kiwi:false,result:false});if(idx==tdoa.COMBINED_MAP)
...
if(tdoa.response.likely_lat!=undefined&&tdoa.response.likely_lon!=undefined){s='most likely: '+tdoa.response.likely_lat+', '+tdoa.response.likely_lon;w3_innerHTML('id-tdoa-result',w3_text('|color: hsl(0, 0%, 70%)',s));url='https://google.com/maps/place/'+tdoa.response.likely_lat+','+tdoa.response.likely_lon;w3_innerHTML('id-tdoa-gmap-link',w3_link('',url,w3_icon('w3-text-aqua','fa-map-marker',18)));}}}
...
function tdoa_day_night_visible_cb(path,checked,first){if(first)return;if(!tdoa.day_night)return;checked=w3_checkbox_get(path);if(tdoa.leaflet){if(checked){tdoa.day_night.addTo(tdoa.kiwi_map);tdoa.map_layers.push(tdoa.day_night);}else{tdoa.day_night.remove();}}else{tdoa.day_night.setMap(checked?tdoa.kiwi_map:null);}}
...
function TDoA_blur(){tdoa_waterfall_close();tdoa_clear_all_hosts();ext_set_data_height();zoom_center=0.5;if(dbgUs)w3_zIndex('id-ext-controls',tdoa.cpanel_zIndex);if(ext_get_optbar()=='optbar-off'&&tdoa.optbar!='optbar-off')
...
function tdoa_preview_click(host,ev){if(tdoa.wf_ws!=null){tdoa.wf_host=host;tdoa.wf_ws.close();}else{var auto=(wf.aper==kiwi.APER_AUTO);tdoa.aper_save=wf.aper;tdoa.maxdb_save=auto?wf.save_maxdb:maxdb;tdoa.mindb_un_save=auto?wf.save_mindb_un:mindb_un;tdoa_wf_preview(host);}}
...
function tdoa_waterfall_close(){kiwi_clearTimeout(tdoa.preview_timeo);if(tdoa.wf_ws!=null&&tdoa.wf_host!=null){var host=tdoa.wf_host;tdoa.wf_host=null;tdoa_wf_preview(host);}else{tdoa.wf_ws=tdoa.wf_host=null;kiwi.wf_preview_mode=false;var auto=(wf.aper==kiwi.APER_AUTO);if(isArg(tdoa.maxdb_save))wf.save_maxdb=maxdb=tdoa.maxdb_save;if(isArg(tdoa.mindb_un_save))wf.save_mindb_un=mindb_un=tdoa.mindb_un_save;if(isArg(tdoa.aper_save))wf_aper_cb('wf.aper',tdoa.aper_save);if(!tdoa.wf_conn_bad){w3_clearInnerHTML('id-tdoa-submit-status');}
...
tdoa.wf_up=false;}
function tdoa_wf_preview(h){tdoa.wf_url=h.hp;tdoa.wf_url_full=h.h+':'+h.p;tdoa.wf_id_snr=h.id_snr;w3_innerHTML('id-tdoa-submit-status','Waterfall preview: '+h.id_snr+' ('+tdoa.wf_url+')');tdoa.wf_ws=open_websocket('W/F',function(){tdoa.preview_lines=0;tdoa.wf_ws.send("SET auth t=kiwi");tdoa.wf_ws.send("SERVER DE CLIENT openwebrx.js W/F");tdoa.wf_ws.send("SET ident_user=TDoA_preview");tdoa.wf_ws.send("SET send_dB=1");tdoa.wf_ws.send('SET zoom='+zoom_level+' start='+x_bin);tdoa.wf_ws.send("SET maxdb=0 mindb=-100");tdoa.wf_ws.send("SET wf_speed=3");tdoa.preview_timeo=setTimeout(function(){if(tdoa.wf_ws)tdoa.wf_ws.close();},10000);},null,null,tdoa_waterfall_add_queue,on_ws_error,tdoa_waterfall_close,{url:tdoa.wf_url_full,new_ts:true,qs:'',all_msg_cb:tdoa_all_msg_cb});tdoa.wf_conn_bad=false;kiwi.wf_preview_mode=true;}
