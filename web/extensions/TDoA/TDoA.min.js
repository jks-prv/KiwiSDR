...
/* web/extensions/TDoA/TDoA.min.js (web/extensions/TDoA/TDoA.js = Sat Aug 16 09:31:27 2025) */
...
if(!tdoa.old_algorithm)s+=",'new',true";if(tdoa.kml)s+=",'kml',1";s+=')';if(tdoa.rerun)s+='&rerun='+tdoa.response.key;if(tdoa.devl)s+='&devl=1';tdoa.last_result_map_select=tdoa.KIWI_MAP;tdoa.response={};tdoa.response.seq=0;tdoa.response.key=(Date.now()%100000).leadingZeros(5);kiwi_ajax_progress(tdoa.url_base+'php/tdoa.php'+tdoa.a[0]+'&key='+tdoa.response.key+s,function(json){},0,0,'',0);var expire=Date.now()+200000;tdoa.progress_interval=setInterval(function(){if(tdoa.response.key){kiwi_ajax(tdoa.url_files+tdoa.response.key+'/progress.json',function(json){tdoa_protocol_response_cb(json);if(tdoa.response.seq==tdoa.SEQ_DONE){kiwi_clearInterval(tdoa.progress_interval);}});}
...
tdoa.wf_up=false;}
function tdoa_wf_preview(h){tdoa.wf_url=h.hp;tdoa.wf_id_snr=h.id_snr;w3_innerHTML('id-tdoa-submit-status','Waterfall preview: '+h.id_snr+' ('+tdoa.wf_url+')');tdoa.wf_ws=open_websocket('W/F',function(){tdoa.preview_lines=0;tdoa.wf_ws.send("SET auth t=kiwi");tdoa.wf_ws.send("SERVER DE CLIENT openwebrx.js W/F");tdoa.wf_ws.send("SET ident_user=TDoA_preview");tdoa.wf_ws.send("SET send_dB=1");tdoa.wf_ws.send('SET zoom='+zoom_level+' start='+x_bin);tdoa.wf_ws.send("SET maxdb=0 mindb=-100");tdoa.wf_ws.send("SET wf_speed=3");tdoa.preview_timeo=setTimeout(function(){if(tdoa.wf_ws)tdoa.wf_ws.close();},10000);},null,null,tdoa_waterfall_add_queue,on_ws_error,tdoa_waterfall_close,{url:tdoa.wf_url,new_ts:true,qs:'',all_msg_cb:tdoa_all_msg_cb});tdoa.wf_conn_bad=false;kiwi.wf_preview_mode=true;}
