...
/* web/extensions/ext.min.js (web/extensions/ext.js = Fri Apr 18 07:45:14 2025) */
...
var ext_zoom={TO_BAND:0,IN:1,OUT:-1,ABS:2,WHEEL:3,CUR:4,NOM_IN:8,MAX_IN:9,MAX_OUT:-9,TO_PASSBAND:null};function ext_tune(freq_dial_kHz,mode,zoom,zlevel,low_cut,high_cut,opt){if(isUndefined(freq_dial_kHz))freq_dial_kHz=freq_displayed_Hz / 1000;var freq_dial_bb_kHz=kiwi_freq_without_offset_kHz(freq_dial_kHz);var pb_specified=(isArg(low_cut)&&isArg(high_cut));console.log('ext_tune: '+freq_dial_bb_kHz+', '+mode+', '+zoom+', '+zlevel);extint.ext_is_tuning=true;freqmode_set_dsp_kHz(freq_dial_bb_kHz,mode,opt);if(pb_specified)ext_set_passband(low_cut,high_cut);if(isArg(zoom)){zoom_step(zoom,Math.round(+zlevel));}else{zoom_step(ext_zoom.TO_BAND);}
