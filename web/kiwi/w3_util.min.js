...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Mon Apr 14 07:28:51 2025) */
...
function w3_disable(el_id,disable){if(!isArg(disable))disable=true;el=w3_el(el_id);w3_set_props(el,'w3-disabled',disable);if(isDefined(el.nodeName)&&(el.nodeName=='SELECT'||el.nodeName=='INPUT')){try{if(disable)
el.setAttribute('disabled','');else
el.removeAttribute('disabled');}catch(ex){console.log('w3_disable:Attribute');console.log(ex);}}
return el;}
function w3_disable_multi(el_id,disable){if(!isArg(disable))disable=true;w3_els(el_id,function(el,i){w3_set_props(el,'w3-disabled',disable);if(isDefined(el.nodeName)&&(el.nodeName=='SELECT'||el.nodeName=='INPUT')){try{if(disable)
el.setAttribute('disabled','');else
el.removeAttribute('disabled');}catch(ex){console.log('w3_disable:Attribute');console.log(ex);}}});}
