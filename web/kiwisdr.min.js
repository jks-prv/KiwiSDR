...
/* web/extensions/ext.min.js (web/extensions/ext.js = Sat Jul 25 14:15:19 2026) */
...
w3_visible('id-ext-controls',false);extint_blur_prev(1);w3_call(extint.hide_func);w3_select_value('id-select-ext',-1);resize_waterfall_container();extint.displayed=false;freqset_select();}
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Sat Jul 25 14:16:37 2026) */
...
function canvas_log_int(s){var el;if(!kiwi.canvas_log_init){el=w3_el('id-news');el.style.top='';el.style.bottom='';el.style.left='';el.style.right='';if(kiwi_isMobile()){var w=screen.width;var h=screen.height;if(w>=600&&h>=950){el.style.bottom=0;el.style.right=0;el.style.width='350px';el.style.height='400px';}else{el.style.top=0;el.style.right=0;el.style.width='150px';el.style.height='150px';}}else{el.style.bottom=0;el.style.left=0;el.style.width='350px';el.style.height='300px';}
...
function preventDefault(ev){ev.preventDefault();return false;}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Sat Jul 25 14:16:37 2026) */
...
function w3_apply(func,args_a){var rv;if(isNoArg(func))return rv;try{if(isString(func)){var f=getVarFromString(func);if(isFunction(f)){rv=f.apply(null,args_a);}else{}}else
if(isFunction(func)){rv=func.apply(null,args_a);}else{console.log('w3_call: func not a string or function');console.log(kiwi_JSON(func));}}catch(ex){console.log('w3_call: while in func this exception occured:');console.log(ex);}
return rv;}
function w3_call(func){var rv;if(isNoArg(func))return rv;var args_a=Array.from(arguments);args_a.shift();rv=w3_apply(func,args_a);return rv;}
...
return el;}
function w3_create_appendElement(el_parent,el_type,html,id,psa){var el_child=document.createElement(el_type);if(isString(id))el_child.id=id;if(isString(psa))w3_set_psa(el_child,psa);w3_innerHTML(el_child,html);w3_el(el_parent).appendChild(el_child);return el_child;}
...
function w3_add(el_id,props){if(!el_id||isEmptyString(props))return null;var first_el=null;var el=w3_el(el_id);if(!el)return null;if(first_el)return;if(!el||!props)return null;props=props.replace(/\s+/g,' ').split(' ');props.forEach(function(p){el.classList.add(p);});if(!first_el)first_el=el;return first_el;}
function w3_add_by_selector(selector,props){var node_list=document.querySelectorAll(selector);if(isNull(node_list))return;node_list.forEach(function(el,i){w3_add(el,props);});}
function w3_remove(el_id,props){if(!el_id||isEmptyString(props))return null;var first_el=null;var el=w3_el(el_id);if(!el)return null;if(!first_el)first_el=el;if(!el||!props)return null;props=props.replace(/\s+/g,' ').split(' ');props.forEach(function(p){el.classList.remove(p);});return first_el;}
function w3_remove_by_selector(selector,props){var node_list=document.querySelectorAll(selector);if(isNull(node_list))return;node_list.forEach(function(el,i){w3_remove(el,props);});}
...
function w3_remove_then_add_cond_by_selector(selector,cond,t_props,f_props){var node_list=document.querySelectorAll(selector);if(isNull(node_list))return;node_list.forEach(function(el,i){w3_remove_then_add_cond(el,cond,t_props,f_props);});}
function w3_func_by_selector(selector,func){var args=Array.from(arguments);args.shift();args.shift();console.log(args);var node_list=document.querySelectorAll(selector);console.log(node_list);if(isNull(node_list))return;node_list.forEach(function(el,i){console.log(i+' id='+el.id);var a=args;a.unshift(el);console.log(a);w3_apply(func,a);});}
...
function w3_set_psa(el,psa){el=w3_el(el);if(!el||isEmptyString(psa))return null;psa=psa.split('/')[0];psa=psa.split('|');var props=psa[0];var styles=psa[1];var attrs=psa[2];if(isArg(props))w3_add(el,props);if(isArg(styles))el.style=styles;}
function w3int_init(){}
...
if(i==narg-1&&last_halign_end)prop+='w3-halign-end';if(psa_inner!=''){var psa3r='';var a1=psa3.right.split(' ');a1.forEach(function(p1){if(dump)console.log('N:prop i='+(im1d2-0)+' p1=');var a2=p1.split(':');if(dump)console.log(a2);if(a2.length==2){var n=+a2[0];if(dump)console.log('i='+i+' n='+n+' '+(n==(im1d2-0)));psa3r=w3_sb(psa3r,(n==(im1d2-0))?a2[1]:'');}else{psa3r=w3_sb(psa3r,p1);}});if(dump)console.log('w3_inline_percent prop='+prop+' style='+style);psa=w3_psa(psa3r,prop,style);}else{psa=w3_psa(psa3.right,prop,style);}
...
kiwi_get_init_settings();if(!no_geoloc)kiwi_geolocate();freq_memory_init();init_top_bar();init_rx_photo();freq_memory_menu_init();keyboard_shortcut_init();confirmation_panel_init();ext_panel_init();place_panels();init_panels();okay_wf_init=true;confirmation_panel_init2();smeter_init();time_display_setup('id-topbar-R-container');right_click_menu_init();window.setInterval(send_keepalive,5000);window.addEventListener('resize',openwebrx_resize,w3.BUBBLING);if(param_nocache){console.log('### nocache='+(nocache?1:0));}
...
function openwebrx_resize(from,delay){if(isNumber(delay)){setTimeout(function(){openwebrx_resize(from);},delay);return;}
from=(isString(from)&&from.startsWith('orient'))?from:'event';resize_wf_canvases();resize_waterfall_container();extint_environment_changed({resize:1,passband_screen_location:1});resize_scale(from);position_top_bar();owrx.resize_seq++;mdev_log_resize();spectrum_dB_bands();}
function position_top_bar(){var w=window.innerWidth;var w_min=kiwi.MIN_WIDTH_TOPBAR;var narrow=(w<w_min);var info=w3_boundingBox_children('id-topbar-L-container');var owner=w3_boundingBox_children('id-topbar-ML-container');var time=w3_boundingBox_children('id-topbar-R-container');w3_el('id-topbar-L-container').style.minWidth=px(info.x2);if(owner.w)
...
function open_rx_photo(){owrx.rx_photo_state=1;w3_opacity("id-rx-photo-desc",1);w3_opacity("id-rx-photo-title",1);animate_to("id-top-photo-clip","maxHeight","px",owrx.rx_photo_total_height,0.93,kiwi_isMobile()?1:1000,60,function(){resize_waterfall_container();});w3_hide('id-topbar-arrow-down');w3_show_block('id-topbar-arrow-up');}
function close_rx_photo(){owrx.rx_photo_state=0;animate_to("id-top-photo-clip","maxHeight","px",owrx.top_bar_cur_height,0.93,kiwi_isMobile()?1:1000,60,function(){resize_waterfall_container();});w3_show_block('id-topbar-arrow-down');w3_hide('id-topbar-arrow-up');}
...
var scale_ctx,band_ctx,dx_ctx;var pb_adj_cf,pb_adj_cf_ttip,pb_adj_lo,pb_adj_lo_ttip,pb_adj_hi,pb_adj_hi_ttip,pb_adj_car,pb_adj_car_ttip;var scale_canvas,band_canvas,dx_div,dx_canvas;function scale_setup(){w3_el('id-scale-container').addEventListener("mouseover",scale_container_mouseover,w3.BUBBLING);w3_el('id-scale-container').addEventListener("mouseout",scale_container_mouseout,w3.BUBBLING);scale_canvas=w3_el("id-scale-canvas");scale_ctx=scale_canvas.getContext("2d");add_scale_listener(scale_canvas);pb_adj_car=w3_el("id-pb-adj-car");pb_adj_car.innerHTML='<span id="id-pb-adj-car-ttip" class="class-passband-adjust-car-tooltip cl-tooltip-text cl-tooltip-down"></span>';w3_title(pb_adj_car,'shift for BFO');pb_adj_car_ttip=w3_el("id-pb-adj-car-ttip");add_scale_listener(pb_adj_car);add_shift_listener(pb_adj_car);pb_adj_lo=w3_el("id-pb-adj-lo");pb_adj_lo.innerHTML='<span id="id-pb-adj-lo-ttip" class="class-passband-adjust-cut-tooltip cl-tooltip-text cl-tooltip-down"></span>';w3_title(pb_adj_lo,'shift for PBT');pb_adj_lo_ttip=w3_el("id-pb-adj-lo-ttip");add_scale_listener(pb_adj_lo);add_shift_listener(pb_adj_lo);pb_adj_hi=w3_el("id-pb-adj-hi");pb_adj_hi.innerHTML='<span id="id-pb-adj-hi-ttip" class="class-passband-adjust-cut-tooltip cl-tooltip-text cl-tooltip-down"></span>';w3_title(pb_adj_hi,'shift for PBT');pb_adj_hi_ttip=w3_el("id-pb-adj-hi-ttip");add_scale_listener(pb_adj_hi);add_shift_listener(pb_adj_hi);pb_adj_cf=w3_el("id-pb-adj-cf");pb_adj_cf.innerHTML='<span id="id-pb-adj-cf-ttip" class="class-passband-adjust-cf-tooltip cl-tooltip-text cl-tooltip-down"></span>';w3_title(pb_adj_cf,'shift for PBT');pb_adj_cf_ttip=w3_el("id-pb-adj-cf-ttip");add_scale_listener(pb_adj_cf);add_shift_listener(pb_adj_cf);band_canvas=w3_el("id-band-canvas");band_ctx=band_canvas.getContext("2d");add_canvas_listener(band_canvas);dx_div=w3_el('id-dx-container');add_canvas_listener(dx_div);dx_canvas=w3_el("id-dx-canvas");dx_ctx=dx_canvas.getContext("2d");add_canvas_listener(dx_canvas);owrx.dx_database=w3_el("id-dx-database");w3_add(owrx.dx_database,'w3-width-zero');w3_innerHTML(owrx.dx_database,w3_button('id-dx-db w3-padding-tiny w3-grey-white w3-font-12px'+'||title="click to change dx label database"','','dx_database_cycle_cb'));resize_scale('setup');}
...
function scale_carfreq_from_px(x,incl_PBO){var visible_range=get_visible_freq_range();var offset=incl_PBO?passband_offset():0;var f=visible_range.start+visible_range.bw*(x / wf_container.clientWidth);return f-offset;}
function canvas_freq_upd(from,evt,x){var area,fnew,snap=false;if((evt.shiftKey&&!(evt.ctrlKey||evt.altKey))||owrx.wf_snap){var fold=canvas_get_dspfreq(x);var b=find_band(fold);var rv=freq_step_amount(b);var step_Hz=rv.step_Hz;var trunc=fold / step_Hz;fnew=Math.round(trunc)*step_Hz;freqmode_set_dsp_kHz(fnew/1000,null);snap=true;}else{var incl_PBO=(owrx.rcm_snap_sel!=owrx.RCM_CAR||ext_mode(cur_mode).CW);fnew=Math.round(scale_carfreq_from_px(x,incl_PBO));}
var pb=freq_passband(fnew);var margin=Math.round(g_range.bw*0.05);var lo_lim=g_range.start+margin;var hi_lim=g_range.end-margin;if(pb.lo<=lo_lim){area=owrx.LEFT_MARGIN;}else
...
if(scale_canvas_params.drag){rv=demodulators[0].envelope.drag_move(x);event_handled|=rv.event_handled;if(!event_handled||rv.fset!=owrx.FSET_PB_CHANGE){var deltaX=scale_canvas_params.last_x-x;var area=canvas_freq_upd(owrx.FSET_SCALE_DRAG,evt,x);if(area!=owrx.NOT_MARGIN&&mobileOpt()){var dbins=norm_to_bins(deltaX / waterfall_width);waterfall_pan_canvases(-dbins);}else{scale_canvas_params.last_x=x;}}}}
...
scale_canvas_params.drag=false;var event_handled=false;if(scale_canvas_params.mouse_down==true){event_handled|=demodulators[0].envelope.drag_end(x);if(!event_handled){canvas_freq_upd(owrx.FSET_SCALE_END,evt,x);}}
...
function init_wf_container(){wf_container=w3_el("id-waterfall-container");waterfall_width=wf_container.clientWidth;canvas_annotation=create_canvas('id-annotation-canvas',wf_fft_size,wf_canvas_default_height,waterfall_width,wf_canvas_default_height);canvas_annotation.style.zIndex=1;wf_container.appendChild(canvas_annotation);add_canvas_listener(canvas_annotation);w3_el('id-kiwi-body').addEventListener('keyup',mouse_freq_remove,w3.BUBBLING);annotation_div=w3_el('id-annotation-div');add_canvas_listener(annotation_div);annotation_div.style.pointerEvents='none';canvas_phantom=w3_el('id-phantom-canvas');add_canvas_listener(canvas_phantom);canvas_phantom.style.width=px(wf_container.clientWidth);add_wf_canvas();spec.container=w3_el('id-spec-container');var spec_width=waterfall_width;spec.canvas=create_canvas('id-spec-canvas',wf_fft_size,spec.height_spectrum_canvas,spec_width,spec.height_spectrum_canvas);spec.container.appendChild(spec.canvas);spec.ctx=spec.canvas.ctx;add_canvas_listener(spec.canvas);spec.ctx.font="10px sans-serif";spec.ctx.textBaseline="middle";spec.ctx.textAlign="left";spec.text_canvas=create_canvas('id-spec-text-canvas',spec_width,spec.height_spectrum_canvas,spec_width,spec.height_spectrum_canvas);spec.container.appendChild(spec.text_canvas);spec.text_ctx=spec.text_canvas.ctx;add_canvas_listener(spec.text_canvas);spec.pb_canvas=create_canvas('id-spec-pb-canvas',wf_fft_size,spec.height_spectrum_canvas,spec_width,spec.height_spectrum_canvas);spec.container.appendChild(spec.pb_canvas);spec.pb_ctx=spec.pb_canvas.ctx;add_canvas_listener(spec.pb_canvas);spec.pb_ctx.font="10px sans-serif";spec.pb_ctx.textBaseline="middle";spec.pb_ctx.textAlign="left";spec.pb_ctx.x1=spec.pb_ctx.x2=null;spec.pb_ctx.seq=0;spec.af_left=50;spec.af_margins=spec.af_left*2;spec.af_canvas=create_canvas('id-spec-af-canvas',wf_fft_size,spec.height_spectrum_canvas,spec_width-spec.af_margins,spec.height_spectrum_canvas);spec.container.appendChild(spec.af_canvas);spec.af_canvas.style.left=px(spec.af_left);spec.af_canvas.style.position='absolute';spec.af_ctx=spec.af_canvas.ctx;add_canvas_listener(spec.af_canvas);var s='<span id="id-spec-dB-ttip" class="class-spectrum-dB-tooltip cl-tooltip-text cl-tooltip-down"></span>';spec.dB=w3_create_appendElement(spec.container,'div',s,'id-spec-dB');w3_add(spec.dB,'class-spectrum-dB cl-tooltip');spec.dB.style.height=px(spec.height_spectrum_canvas);spec.dB.style.width=px(spec_width);add_canvas_listener(spec.dB);s=w3_div('id-spec-size w3-flex-col||title="adjust spectrum size"',w3_icon('w3-custom-events w3-pointer w3-margin-B-1 w3-halign-center','fa-arrow-up',15,'orange'),w3_icon('w3-custom-events w3-pointer w3-margin-T-1 w3-halign-center','fa-arrow-down',15,'cyan'));spec.size=w3_create_appendElement(spec.container,'div',s,'id-spec-size-container');if(kiwi_isMobile()){spec.size.style.width='50px';}
w3_do_when_rendered('id-spec-dB-ttip',function(el){spec.dB_ttip=el;});document.addEventListener("mousemove",mouse_freq_add,w3.BUBBLING);}
...
if(evt.shiftKey&&(evt.ctrlKey||evt.altKey)){canvas_ignore_mouse_event=true;if(owrx.debug_drag)console.log('CSD-lookup IME=set-true');freq_database_lookup(canvas_get_dspfreq(x),owrx.rcm_lookup);}else
...
function canvas_drag(evt,idx,x,y,clientX,clientY){if(!waterfall_setup_done)return;if(spectrum_tooltip_update(owrx.EV_DRAG,evt,clientX,clientY)){return;}
owrx.drag_count=!isNumber(owrx.drag_count)?0:(owrx.drag_count+1);if(owrx.scale_canvas.mouse_out){scale_canvas_drag(evt,x,y);return;}
...
if(!owrx.ignore_freq_tune_onmouseup){if(owrx.debug_drag)canvas_log('*click*');if((evt.shiftKey&&!(evt.ctrlKey||evt.altKey))||owrx.wf_snap){var fold=canvas_get_dspfreq(x);var b=find_band(fold);var rv=freq_step_amount(b);var step_Hz=rv.step_Hz;var trunc=fold / step_Hz;var fnew=Math.round(trunc)*step_Hz;freqmode_set_dsp_kHz(fnew/1000,null);}else{var incl_PBO=(owrx.rcm_snap_sel!=owrx.RCM_CAR||ext_mode(cur_mode).CW);var car_freq=canvas_get_carfreq(x,incl_PBO);demodulator_set_frequency(owrx.FSET_MOUSE_CLICK,car_freq);}}}}else{canvas_end_drag2();}
...
if(kHz<b.MW_hi){f=Math.round(kHz_r1k/b._9_10)*b._9_10;console.log('MW kHz='+kHz+' f='+f);var mwlist_area=[0,1,3,2];url+="www.mwlist.org/mwlist_quick_and_easy.php?area="+mwlist_area[b.ITU_region]+"&kHz="+f.toFixed(0);}else{f=Math.round(kHz_r1k/5)*5;url="http://www.short-wave.info/index.php?freq="+f.toFixed(0)+"&timbus=NOW&ip="+client_public_ip+"&porm=4";}}
...
mobile_scale_control_panel(mobile);mobile_orientation_adjustments(mobile);},500);}
function mobile_orientation_adjustments(mobile){w3_el('id-dx-database').style.left=mobile.phone?'0':'16px';}
...
var spec={height_spectrum_canvas:200,tooltip_offset:100,canvas:null,ctx:null,spectrum_image:null,colormap:null,colormap_transparent:null,dB:null,dB_ttip:null,update:0,last_update:0,need_update:0,size_drag:false,size_icon_h:8+15+15+8,dB_SCALE_WIDTH:30,dB_scale_w:0,dB_scale_timer:null,need_clear_avg:0,clear_avg:0,avg:[[],[]],spec_color:'#d3d3d3',SPEC_RF:0,SPEC_AF:1,TRACE_0:0,TRACE_1:1,NONE:0,RF:1,AF:2,CHOICES:3,source:0,PEAK_OFF:0,PEAK_ON:1,PEAK_HOLD:2,peak_show:[0,0],peak_clear_spec:[0,0],peak_clear_btn:[0,0],peak:[[[],[]],[[],[]]],peak1_color:'yellow',peak2_color:'magenta',peak_alpha:1.0,dB_bands:[],redraw_dB_scale:false,};function spectrum_init(){spec.dB_scale_w=kiwi_isMobile()?0:spec.dB_SCALE_WIDTH;spec.colormap=spec.ctx.createImageData(1,spec.canvas.height);spec.colormap_transparent=spec.ctx.createImageData(1,spec.canvas.height);update_maxmindb_sliders();spectrum_dB_bands();var spectrum_update_rate_Hz=10;setInterval(function(){spec.update++;},1000 / spectrum_update_rate_Hz);spec.spectrum_image=spec.ctx.createImageData(spec.canvas.width,spec.canvas.height);spec.size.addEventListener('mousedown',spectrum_resize_mousedown,w3.CAPTURING);if(kiwi_isMobile()){spec.size.addEventListener('touchstart',spectrum_resize_mousedown,w3.CAPTURING);}
...
function spectrum_set_height(h){if(h<=spec.size_icon_h||h>window.innerHeight)return;console.log('spectrum_set_height '+h);spec.height_spectrum_canvas=h;spec.colormap=spec.ctx.createImageData(1,h);spec.colormap_transparent=spec.ctx.createImageData(1,h);spec.spectrum_image=spec.ctx.createImageData(spec.canvas.width,h);var h_px=px(h);spec.container.style.height=h_px;spec.dB.style.height=h_px;spec.canvas.height=h;spec.canvas.style.height=h_px;spec.text_canvas.height=h;spec.text_canvas.style.height=h_px;spec.pb_canvas.height=h;spec.pb_canvas.style.height=h_px;spec.af_canvas.height=h;spec.af_canvas.style.height=h_px;openwebrx_resize();}
function spectrum_resize_mousedown(ev){spec.size_drag=true;w3_add(spec.dB_ttip,'w3-panel-override-hide');window.addEventListener('mousemove',spectrum_resize_mousemove,w3.CAPTURING);if(kiwi_isMobile()){window.addEventListener('touchmove',spectrum_resize_touchmove,w3.CAPTURING);}
window.addEventListener('mouseup',spectrum_resize_mouseup,{capture:true,once:true});if(kiwi_isMobile()){window.addEventListener('touchend',spectrum_resize_touchend,{capture:true,once:true});}}
function spectrum_resize_move(y){if(!spec.size_drag)return;spectrum_set_height(y+spec.size_icon_h/2);}
function spectrum_resize_mousemove(ev){spectrum_resize_move(ev.clientY);}
function spectrum_resize_touchmove(ev){var y=Math.round(ev.targetTouches[0].pageY);spectrum_resize_move(y);}
function spectrum_resize_mouseup(ev){spectrum_dB_bands();spec.size_drag=false;w3_remove(spec.dB_ttip,'w3-panel-override-hide');window.removeEventListener('mousemove',spectrum_resize_mousemove,w3.CAPTURING);if(kiwi_isMobile()){window.removeEventListener('touchmove',spectrum_resize_touchmove,w3.CAPTURING);}}
function spectrum_resize_touchend(ev){spectrum_resize_mouseup(ev);}
...
spec.redraw_dB_scale=spec.clear_avg=true;spectrum_show_dB_scale();w3_call('colormap_aper');}
function spectrum_show_dB_scale(){if(kiwi_isMobile()){spec.dB_scale_w=spec.dB_SCALE_WIDTH;console.log('spectrum_dB_bands: MOBILE dB_scale ON redraw_dB_scale TRUE');spec.redraw_dB_scale=spec.clear_avg=true;kiwi_clearTimeout(spec.dB_scale_timer);spec.dB_scale_timer=setTimeout(function(){spec.dB_scale_w=0;console.log('spectrum_dB_bands: MOBILE dB_scale OFF redraw_dB_scale TRUE');spec.redraw_dB_scale=spec.clear_avg=true;},2000);}}
...
spectrum_show_dB_scale();return adj_pb;}
function spectrum_update(data){if(spec.source==spec.NONE||spec.size_drag){return;}
var i,trace,x,y,z,band;spec.last_update=spec.update;var spec_width=w3_el("id-waterfall-container").clientWidth;var ctx=spec.ctx;var sw2=spec.canvas.width;var ratio=(spec_width-spec.dB_scale_w)/ Math.min(spec_width,kiwi.NOM_WIDTH_SPEC);var text_w=spec.dB_scale_w*ratio;var sw1=Math.round(sw2*ratio);var sw=(spec.source==spec.RF)?sw1:sw2;var sh=spec.canvas.height;w3_visible('id-spec-af-canvas',spec.source==spec.AF);ctx.fillStyle="black";ctx.fillRect(0,0,sw2,sh);spec.af_ctx.fillStyle="black";spec.af_ctx.fillRect(0,0,spec.canvas.width,sh);ctx.fillStyle="lightGray";spec.af_ctx.fillStyle="lightGray";for(i=0;i<spec.dB_bands.length;i++){band=spec.dB_bands[i];y=Math.round(band.norm*sh);ctx.fillRect(0,y,sw1,1);spec.af_ctx.fillRect(0,y,sw2,1);}
...
if(spec.redraw_dB_scale){if(spec.dB_scale_w!=0){var txc,txw,tbs,txs;var old=0;if(old){txc=spec.ctx;txw=spec.canvas.width;tbs=sw;txs=sw1+3;}else{txc=spec.text_ctx;txw=spec_width;var ratio_i=1/ratio;tbs=txw-spec.dB_scale_w;txs=tbs+1;}
txc.clearRect(0,0,txw,sh);for(x=tbs;x<txw;x++){txc.putImageData(spec.colormap_transparent,x,0,0,0,1,sh);}
if(old){txc.save();txc.font=sprintf("%dpx sans-serif",Math.floor(10*ratio));}else{txc.font='13px sans-serif';}
txc.textBaseline="middle";txc.textAlign="left";txc.textBaseline='alphabetic';txc.fillStyle='white';var prev_y=0;var len=spec.dB_bands.length;for(i=0;i<len;i++){band=spec.dB_bands[i];y=Math.round(band.norm*sh);if(i<(len-1)||(y-prev_y)>12)
txc.fillText(band.dB,txs,y-4);prev_y=y;}
if(old)txc.restore();}else{spec.text_ctx.clearRect(0,0,spec_width,sh);}
...
if(cfg.init.tab)keyboard_shortcut_nav(kiwi.tab_s[cfg.init.tab]);openwebrx_resize();waterfall_setup_done=1;if(0)
w3_func_by_selector('.w3-scroll-mkr',function(el,a,b,c){console.error(el);console.log(kiwi_JSON([a,b,c]));if(0)
w3_create_appendElement(el,'div',w3_icon('w3-custom-events w3-pointer w3-margin-B-1','fa-arrow-down',15,'cyan'),'id-jks','w3-icon-fixed');},11,22,33);if(0){w3_add('id-control','w3-relative');w3_innerHTML('id-control-overlay',w3_icon('id-jks2 w3-icon-fixed w3-custom-events|top:170px; left:360px','fa-arrow-up',15,'cyan'),w3_div('w3-aqua w3-absolute|width:3px; height:119px; top:183px; left:365.5px'),w3_icon('id-jks2 w3-icon-fixed w3-custom-events|top:300px; left:360px','fa-arrow-down',15,'cyan'));}}
...
function resize_wf_canvases(){waterfall_width=wf_container.clientWidth;new_width=px(waterfall_width);var zoom_value=0;wf_canvases.forEach(function(p){p.style.width=new_width;p.style.left=zoom_value;});canvas_annotation.width=waterfall_width;canvas_annotation.style.width=new_width;canvas_phantom.style.width=new_width;canvas_phantom.style.left=zoom_value;var spec_w=px(waterfall_width);spec.canvas.style.width=spec_w;spec.pb_canvas.style.width=spec_w;spec.af_canvas.style.width=px(waterfall_width-spec.af_margins);spec.text_canvas.width=waterfall_width;spec.text_canvas.style.width=spec_w;if(wf.audioFFT_active&&!kiwi_isMobile()){var reason;if(wf.no_wf){reason='when \"no_wf\" URL option used.';}else{if(wf_chans==0){reason='because all waterfalls disabled<br>on this Kiwi.';}else{reason='on channels '+wf_chans_real+'-'+(rx_chans-1)+' of Kiwis<br>'+'configured for '+rx_chans+' channels.';}}
...
function wf_snap(set){owrx.wf_snap=isUndefined(set)?(owrx.wf_snap^1):(isNull(set)?0:(+set));w3_el('id-waterfall-container').style.cursor=owrx.wf_cursor=owrx.wf_snap?'col-resize':'crosshair';kiwi_storeWrite('wf_snap',owrx.rcm_snap_sel);}
...
var band_menu=[];function mk_band_menu(){band_menu=[];owrx.last_selected_band=0;var i,op=0,service=null,last_svc_name=null;var obj={};band_menu[op++]=null;var ITU_region=cfg.init.ITU_region+1;var _dxcfg=dx_cfg_db();if(!_dxcfg)return'';var _kiwi_bands=kiwi_bands_db(dx.INIT_BANDS_NO);for(i=0;i<_dxcfg.bands.length;i++){var b1=_dxcfg.bands[i];var b2=_kiwi_bands[i];if((kiwi.isOffset&&!b2.isOffset)||(!kiwi.isOffset&&b2.isOffset))continue;if(!(b1.itu==kiwi.BAND_MENU_ONLY||b1.itu==kiwi.ITU_ANY||b1.itu==ITU_region))continue;if(service!=b1.svc){service=b1.svc;var svc=band_svc_lookup(b1.svc);if(!svc)continue;last_svc_name=svc.o.name.toUpperCase();obj[last_svc_name]=[];band_menu[op++]=null;}
obj[last_svc_name].push(b1.name);band_menu[op]={};band_menu[op].b1=b1;band_menu[op].b2=b2;op++;}
w3_innerHTML('id-select-band-container',w3_select_hier('id-select-band w3-pointer','','select band','',-1,obj,'select_band_cb'));}
...
function dx_database_cb(path,idx,first,opt,from_shortcut){var opt_s=kiwi_JSON(opt);if(opt==0||opt_s=='[object Object]'){opt={menu:1};opt_s='{menu:1}';}else{opt=opt||{};}
if(first)return;dx.list=[];console_log_dbgUs('DX DB-SWITCHED db='+idx+'|'+dx.db+' opt='+opt_s+' from_shortcut='+from_shortcut);if(w3_opt(opt,'toggle')){dx.filter_tod[dx.db]^=1;dx_time_dow_cb(dx.db.toString()+'-filter-tod',dx.filter_tod[dx.db]);}else{dx.db=+idx;if(w3_opt(opt,'open')){dx_show_edit_panel(null,-1,from_shortcut);}else{var displayed=ext_panel_displayed('dx');if(!opt.menu&&displayed){console.log('opt.menu 1');dx_close_edit_panel();}
if(opt.menu||(opt.cycle&&displayed)){console.log('opt.menu 2');dx_show_edit_panel(null,-1,from_shortcut);}}}
kiwi_storeWrite('last_db',dx.db);bands_init();mkscale();mk_band_menu();dx_schedule_update();}
function dx_database_cycle_cb(){dx.db=(dx.db+1)%dx.DB_N;console.log('dx_database_cycle_cb '+dx.db);dx_database_cb('',dx.db,false,{cycle:1});}
...
function test_audio_suspended(){var s1,onclick=null,input=false,ident=kiwi_storeInit('ident','');var m_old=(kiwi.mdev&&!kiwi.mnew);if(cfg.require_id&&ident_illegal(ident)){snd_send('SET require_id=1');s1=w3_input('w3-valign-center//w3-custom-events w3-margin-T-10 w3-font-18px w3-normal w3-padding-1'+'|width:300px|size=20'+' onchange="ident_complete(\'el\', 3)" onkeyup="ident_keyup(this, event, 3)"','Enter your name or callsign <br> to start KiwiSDR','ident-input3');onclick='';input=true;}else
...
function panels_setup(){var el,s,s1,fmt;el=w3_el("id-ident");el.innerHTML=w3_input('w3-label-not-bold/w3-custom-events w3-padding-1||size=20'+' onchange="ident_complete(\'el\', 1)" onkeyup="ident_keyup(this, event, 1)"','Your name or callsign:','ident-input1');el.addEventListener('click',cancelEvent,w3.CAPTURING);var mobile=kiwi_isMobile()?(' inputmode='+dq(kiwi_is_iOS()?'decimal':'tel')+' ontouchstart="popup_keyboard_touchstart(event)" onclick="this.select()"'):'';w3_el("id-control-freq1").innerHTML=w3_inline('w3-halign-space-between/',w3_div('id-freq-cell','<form id="id-freq-form" name="form_freq" action="#" onsubmit="freqset_complete(0, event); return false;">'+
...
function toggle_or_set_hide_bars(set){var data_container_or_spec=(extint.using_data_container||spec.source!=spec.NONE);if(isNumber(set))
owrx.hide_bars=set;else{owrx.hide_bars=(owrx.hide_bars+1)&owrx.HIDE_ALLBARS;if((owrx.hide_bars&owrx.HIDE_TOPBAR)&&data_container_or_spec)
owrx.hide_bars=(owrx.hide_bars+1)&owrx.HIDE_ALLBARS;}
w3_set_props('id-top-container','w3-panel-override-hide',owrx.hide_bars&owrx.HIDE_TOPBAR);w3_set_props('id-band-container','w3-panel-override-hide',owrx.hide_bars&owrx.HIDE_BANDBAR);var bottom=(!data_container_or_spec&&(owrx.hide_bars==owrx.HIDE_BANDBAR||owrx.hide_bars==owrx.HIDE_ALLBARS));w3_remove_then_add_cond_by_selector('.cl-tooltip-text',bottom,'cl-tooltip-bottom cl-tooltip-up','cl-tooltip-down');openwebrx_resize();}
...
var bottom=(!isSpec&&(owrx.hide_bars==owrx.HIDE_BANDBAR||owrx.hide_bars==owrx.HIDE_ALLBARS));w3_remove_then_add_cond_by_selector('.cl-tooltip-text',bottom,'cl-tooltip-bottom cl-tooltip-up','cl-tooltip-down');freqset_select();snd_send('SET spc_='+spec.source);openwebrx_resize('spec');}
...
var divControl;function panel_setup_control(el){divControl=el;el.style.marginBottom='0';var scroll_mkr=' w3-hide w3-scroll-y w3-relative w3-scroll-mkr|height:'+px(kiwi.OPTBAR_CONTENT_HEIGHT);var el2=w3_el('id-control-inner');w3_add(el2,'w3-no-copy-popup');el2.innerHTML=w3_div('id-control-top',w3_div('id-control-freq1'),w3_div('id-control-freq2'),w3_div('id-control-mode w3-margin-T-6'),w3_div('id-control-zoom w3-margin-T-6'),w3_div('id-optbar w3-margin-T-4'))+
w3_div('id-optbar-content w3-margin-T-6 w3-scroll-y|height:'+px(kiwi.OPTBAR_CONTENT_HEIGHT),w3_div('id-optbar-rf'+scroll_mkr),w3_div('id-optbar-wf'+scroll_mkr),w3_div('id-optbar-audio'+scroll_mkr),w3_div('id-optbar-agc w3-hide'),w3_div('id-optbar-user w3-hide w3-scroll-x'),w3_div('id-optbar-stat w3-hide'))+
