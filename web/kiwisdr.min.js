...
var ext_zoom={TO_BAND:0,IN:1,OUT:-1,ABS:2,WHEEL:3,CUR:4,NOM_IN:8,MAX_IN:9,MAX_OUT:-9,TO_PASSBAND:null};function ext_tune(freq_dial_kHz,mode,zoom,zlevel,low_cut,high_cut,opt){if(isUndefined(freq_dial_kHz))freq_dial_kHz=freq_displayed_Hz / 1000;var freq_dial_bb_kHz=kiwi_freq_without_offset_kHz(freq_dial_kHz);var pb_specified=(isArg(low_cut)&&isArg(high_cut));extint.ext_is_tuning=true;freqmode_set_dsp_kHz(freq_dial_bb_kHz,mode,opt);if(pb_specified)ext_set_passband(low_cut,high_cut);if(isArg(zoom)){zoom_step(zoom,Math.round(+zlevel));}else{zoom_step(ext_zoom.TO_BAND);}
...
return{s:s,a:a};};var init_common=function(init){var r,c;if(dbg2)console_nv('init_common',{init},'kiwi.d.p.nrows');if(init==p.INIT_ONCE||init==p.INIT_RESIZE){p.screen=[];p.color=[];p.dirty=[];p.els=[];for(r=0;r<=p.nrows;r++){p.screen[r]=[];p.color[r]=[];}}
...
/* web/kiwi/kiwi_ui.min.js (web/kiwi/kiwi_ui.js = Thu Apr 24 09:56:00 2025) */
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Thu May  1 10:19:24 2025) */
...
Number.prototype.withSign=function(){var n=Number(this);var s=n.toString();return(n<0)?s:('+'+s);};var kHz_s=function(Hz){return(Hz / 1e3).toFixed(3)+'k';};function _10Hz(f){var step=10;if(f>=0){return Math.trunc(f/step)*step;}else{return-(Math.ceil(-f/step)*step);}}
...
if(!el)return;if(opts['scanning']&&extint.scanning)return;var focus=0,select=0,blur=0,dismiss=0;if(opts['mobile']&&kiwi_isMobile())blur=1;else focus=select=1;if(opts['blur'])blur=1;if(opts['dismiss_kybd'])dismiss=1;if(opts['focus_select'])focus=select=1;if(opts['focus_only']){focus=1;select=0;}
...
function w3_disable(el_id,disable,prop){if(!isArg(disable))disable=true;if(!isString(prop))prop='w3-disabled';el=w3_el(el_id);w3_set_props(el,prop,disable);if(isDefined(el.nodeName)&&(el.nodeName=='SELECT'||el.nodeName=='INPUT')){try{if(disable)
el.setAttribute('disabled','');else
el.removeAttribute('disabled');}catch(ex){console.log('w3_disable:Attribute');console.log(ex);}}
return el;}
function w3_disable_multi(el_id,disable,prop){if(!isArg(disable))disable=true;if(!isString(prop))prop='w3-disabled';w3_els(el_id,function(el,i){w3_set_props(el,prop,disable);if(isDefined(el.nodeName)&&(el.nodeName=='SELECT'||el.nodeName=='INPUT')){try{if(disable)
el.setAttribute('disabled','');else
el.removeAttribute('disabled');}catch(ex){console.log('w3_disable:Attribute');console.log(ex);}}});}
function w3_visible(el_id,visible){if(!isArg(visible))visible=true;var el=w3_el(el_id);el.style.visibility=visible?'visible':'hidden';return el;}
...
function w3int_label(psa,text,path,extension){psa=psa||'';text=text||'';extension=extension||'';var dump=psa.includes('w3-dump');if(text==''&&((psa!=''&&psa!='||')||extension!='')){console_nv('$w3int_label',{text},{psa},{extension});}
...
if(trace)console.log('ss='+el.selectionStart+' se='+el.selectionEnd+' tl='+el.textLength);if((el.selectionStart==el.selectionEnd&&el.selectionStart==el.textLength)||(el.selectionStart==0&&el.selectionEnd==el.textLength)||(el.value=='')){if(trace)console.log('w3_input_change...');w3_input_change(path,cb,'kd');w3_dismiss_keyboard(el);}}else
if(el.value==''){w3_input_change(path,cb,'kd');w3_dismiss_keyboard(el);}}
if(ev.key=='Backspace'&&input_any_change&&el.selectionStart==0&&el.selectionEnd==el.value.length){el.value='';w3_input_change(path,cb,'kd');w3_dismiss_keyboard(el);}}
...
w3_dismiss_keyboard(el);w3int_post_action();}
...
/* web/kiwi/kiwi_map.min.js (web/kiwi/kiwi_map.js = Wed Feb  5 19:45:38 2025) */
var kmap={ADD_TO_MAP:true,NO_ADD_TO_MAP:false,DIR_LEFT:true,DIR_RIGHT:false,VISIBLE:true,NOT_VISIBLE:false,_last_last:0};function kiwi_map_init(ext_name,init_latlon,init_zoom,mapZoom_nom,move_cb,zoom_cb){var map_tiles;var maxZoom=19;map_tiles=function(){maxZoom=18;return L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{tileSize:256,zoomOffset:0,attribution:'<a href="https://www.openstreetmap.org/copyright" target="_blank">&copy; OpenStreetMap contributors</a>',crossOrigin:true});};var tiles=map_tiles('hybrid');var map=L.map('id-'+ext_name+'-map',{maxZoom:maxZoom,minZoom:1,doubleClickZoom:false,zoomControl:false}).setView(init_latlon,init_zoom);map.on('baselayerchange',function(e){var tmap=e.target;var center=tmap.getCenter();kiwi_pan_zoom(tmap,[center.lat+0.1,center.lng],-1);kiwi_pan_zoom(tmap,[center.lat,center.lng],-1);});var kmap={ext_name:ext_name,map:map};map.on('click',function(e,kmap){w3_call(ext_name+'_map_click_cb',kmap,e);});map.on('move',function(e,kmap){w3_call(ext_name+'_map_move_cb',kmap,e);});map.on('zoom',function(e,kmap){w3_call(ext_name+'_map_zoom_cb',kmap,e);});map.on('moveend',function(e,kmap){w3_call(ext_name+'_map_move_end_cb',kmap,e);});map.on('zoomend',function(e,kmap){w3_call(ext_name+'_map_zoom_end_cb',kmap,e);});L.control.zoom_Kiwi({position:'topright',zoomNomText:'2',zoomNomLatLon:init_latlon}).addTo(map);tiles.addTo(map);var scale=L.control.scale();scale.addTo(map);scale.setPosition('bottomleft');var day_night=new Terminator();day_night.setStyle({fillOpacity:0.35});day_night.addTo(map);kmap.day_night_interval=setInterval(function(){var t2=new Terminator();day_night.setLatLngs(t2.getLatLngs());day_night.redraw();},60000);day_night._path.style.cursor='grab';var graticule=L.latlngGraticule();graticule.addTo(map);kmap.map_layers=[graticule];kmap.graticule=graticule;kmap.day_night=day_night;kmap.init_latlon=init_latlon;kmap.init_zoom=init_zoom;kmap.mapZoom_nom=mapZoom_nom;return kmap;}
...
/* web/kiwi/waterfall.min.js (web/kiwi/waterfall.js = Sun May  4 20:22:33 2025) */
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Thu Apr  3 08:53:07 2025) */
...
w3_set_value('id-wheel-dev',idx);kiwi_storeWrite('wheel_dev',idx);w3_set_value('id-wheel-poll',owrx.wheel_poll);w3_set_value('id-wheel-fast',owrx.wheel_fast);w3_set_value('id-wheel-unlock',owrx.wheel_unlock);w3_disable_multi('id-wheel-field',idx!=owrx.WHEEL_CUSTOM,'w3-disabled-field');canvas_mousewheel_rlimit_tune=kiwi_rateLimit(canvas_mousewheel_cb,owrx.wheel_poll);}
...
var fracHz=kiwi.offset_frac_Hz;var fnew=freq_displayed_Hz+fracHz;var step_Hz=up_down[cur_mode][sel]*1000;if(step_Hz==0){var b=find_band(fnew);step_Hz=special_step(b,sel,'freqstep_cb');}
...
top=dx_label_top+(gap*(dx_idx&1));dx.post_render[dx_idx]={top:top,ltop:top,x:x};dx.last_f_base=f_base_label_Hz;var color;if(eibi){color=filtered?dx.eibi_colors_light[color_idx]:dx.eibi_colors[color_idx];}else{var c=_dxcfg.dx_type[color_idx].color;if(c=='')c='white';color=filtered?dx.stored_colors_light[color_idx]:c;}
...
function panels_setup(){var el,s,s1,fmt;el=w3_el("id-ident");el.innerHTML=w3_input('w3-label-not-bold/w3-custom-events w3-padding-1||size=20'+' onchange="ident_complete(\'el\', 1)" onkeyup="ident_keyup(this, event, 1)"','Your name or callsign:','ident-input1');el.addEventListener('click',function(evt){return cancelEvent(evt);},true);var mobile=kiwi_isMobile()?(' inputmode='+dq(kiwi_is_iOS()?'decimal':'tel')+' ontouchstart="popup_keyboard_touchstart(event)" onclick="this.select()"'):'';w3_el("id-control-freq1").innerHTML=w3_inline('w3-halign-space-between/',w3_div('id-freq-cell','<form id="id-freq-form" name="form_freq" action="#" onsubmit="freqset_complete(0, event); return false;">'+
...
w3_hr('cl-cpanel-hr')+
w3_div('id-optbar-rf-antsw'));var no_attn=(kiwi.model==kiwi.KiwiSDR_1)&&!cfg.rf_attn_alt;var deny_not_local=false,deny_not_local_or_pwd=false;if(cfg.rf_attn_allow==kiwi.RF_ATTN_ALLOW_LOCAL_ONLY&&ext_auth()!=kiwi.AUTH_LOCAL)deny_not_local=true;if(cfg.rf_attn_allow==kiwi.RF_ATTN_ALLOW_LOCAL_OR_PASSWORD_ONLY&&ext_auth()==kiwi.AUTH_USER)deny_not_local_or_pwd=true;if(no_attn||deny_not_local||deny_not_local_or_pwd){kiwi.rf_attn_disabled=true;w3_disable_multi('id-rf-attn-disable',true);var title;if(no_attn)title='no RF attenuator on KiwiSDR 1';else
if(deny_not_local)title='only available to local connections';else
if(deny_not_local_or_pwd)title='only available to local connections or with password';w3_title_multi('id-rf-attn-disable',title);}
...
function update_wf_stats(){var s='span '+zoom_span_Hz.toUnits(true)+'Hz, '+kiwi.wf_fps.toFixed(0)+' fps';if(kiwi.wf_share)s+=', share mode';w3_innerHTML('id-status-wf',w3_text('w3-text-css-orange','WF'),w3_text('',s));}
...
/* web/openwebrx/audio.min.js (web/openwebrx/audio.js = Wed Mar 19 08:49:17 2025) */
...
audio_channels=audio_mode_iq?2:1;var now,msec;if(audio_compression){if(audio.d){now=Date.now();msec=now-audio.last_msec;audio.last_msec=now;console.log('AC Q'+audio_prepared_buffers.length+' '+audio_channels+'ch '+msec);}
decode_ima_adpcm_e8_i16(data_view,audio_data,audio_data_unsquelched,squelched,bytes,audio_adpcm);samps=bytes*2;}else{if(audio.d){now=Date.now();msec=now-audio.last_msec;audio.last_msec=now;if(isIQ)
