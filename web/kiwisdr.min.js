...
/* web/extensions/ext.min.js (web/extensions/ext.js = Mon Jul 21 13:40:13 2025) */
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Tue Aug  5 11:39:19 2025) */
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Sun Jul 20 20:56:42 2025) */
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Mon Aug 25 12:18:15 2025) */
...
return obj;}}
return cbp;}
function w3int_button(psa,path,text,cb,cb_param){var momentary=psa.includes('w3-momentary');var hold=psa.includes('w3-hold');var custom_events=psa.includes('w3-custom-events');var dump=psa.includes('w3-dump');cb_param=w3_cb_param_encode(cb_param);var ev_cb=function(cbp){return sprintf('"w3int_btn_evt(event,%s,%s,%s);"',sq(path),sq(cb),sq(cbp));};var onclick=cb?('onclick='+ev_cb(cb_param)):'';if(cb&&momentary&&!hold&&!custom_events){onclick+=' onmousedown='+ev_cb(0);onclick+=' ontouchstart='+ev_cb(0);if(dump)canvas_log('w3int_button MOM');}else
if(cb&&hold&&!momentary&&!custom_events){onclick+=' onmousedown='+ev_cb(cb_param);onclick+=' onmouseout='+ev_cb(cb_param);onclick+=' ontouchstart='+ev_cb(cb_param);onclick+=' ontouchend='+ev_cb(cb_param);if(dump)canvas_log('w3int_button HOLD');}else
if(cb&&(custom_events||hold)&&!momentary){onclick+=' onmouseenter='+ev_cb(cb_param);onclick+=' onmouseleave='+ev_cb(cb_param);onclick+=' onmousedown='+ev_cb(cb_param);onclick+=' onmousemove="ignore(event);"';onclick+=' onmouseup="ignore(event);"';onclick+=' ontouchstart='+ev_cb(cb_param);onclick+=' ontouchmove="ignore(event);"';onclick+=' ontouchend='+ev_cb(cb_param);if(dump)canvas_log('w3int_button CUS');}else
if(dump)canvas_log('w3int_button DEF');var default_style=(psa.includes('w3-round')||psa.includes('w3-circle'))?'':' w3-round-6px';var noactive=psa.includes('w3-noactive')?' class-button-noactive w3-ext-btn-noactive':'';var right=psa.includes('w3-btn-right')?'w3-ialign-right':'';var psa3=w3_psa3(psa);var psa_outer=w3_psa(psa3.left,right);var psa_inner=w3_psa(psa3.right,path+' w3-btn w3-ext-btn'+default_style+noactive,'',onclick);if(dump){console.log('w3_button');console.log(psa3);console.log('psa_outer='+psa_outer);console.log('psa_inner='+psa_inner);}
...
return el;}
function w3_icon(psa,fa_icon,size,color,cb,cb_param){var momentary=psa.includes('w3-momentary');var hold=psa.includes('w3-hold');var custom_events=psa.includes('w3-custom-events');var dump=psa.includes('w3-dump');var pointer=(cb&&cb!='')?' w3-pointer':'';var path='id-btn-grp-'+w3int.btn_grp_uniq.toString();w3int.btn_grp_uniq++;var font_size=null;if(isNumber(size)&&size>=0)font_size=px(size);else
if(isString(size))font_size=size;font_size=font_size?(' font-size:'+font_size+';'):'';color=color||'';var c=color.split('|');color='';if(c[0]!='')color=' color:'+c[0]+';';if(c.length>=2&&c[1]!='')color+=' background-color:'+c[1]+';';cb_param=w3_cb_param_encode(cb_param);var ev_cb=function(cbp){return sprintf('"w3int_btn_evt(event, %s, %s, %s)"',sq(path),sq(cb),sq(cbp));};var onclick=cb?('onclick='+ev_cb(cb_param)):'';if(cb&&momentary&&!hold&&!custom_events){onclick+=' onmousedown='+ev_cb(0);onclick+=' ontouchstart='+ev_cb(0);if(dump)canvas_log('w3_icon MOM');}else
if(cb&&hold&&!momentary&&!custom_events){onclick+=' onmousedown='+ev_cb(cb_param);onclick+=' onmouseout='+ev_cb(cb_param);onclick+=' ontouchstart='+ev_cb(cb_param);onclick+=' ontouchend='+ev_cb(cb_param);if(dump)canvas_log('w3_icon HOLD');}else
if(cb&&(custom_events||hold)&&!momentary){onclick+=' onmouseenter='+ev_cb(cb_param);onclick+=' onmouseleave='+ev_cb(cb_param);onclick+=' onmousedown="ignore(event)"';onclick+=' onmousemove="ignore(event)"';onclick+=' onmouseup="ignore(event)"';onclick+=' ontouchstart='+ev_cb(cb_param);onclick+=' ontouchmove="ignore(event)"';onclick+=' ontouchend='+ev_cb(cb_param);if(dump)canvas_log('w3_icon CUS');}else
if(dump)canvas_log('w3_icon DEF');var p=w3_psa(psa,path+' w3-ext-icon'+pointer+' fa '+fa_icon,font_size+color,onclick);var s='<i '+p+'></i>';return s;}
...
/* web/kiwi/noise_filter.min.js (web/kiwi/noise_filter.js = Sat Aug  2 18:14:24 2025) */
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Tue Aug  5 11:43:46 2025) */
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Sat Aug  9 12:02:44 2025) */
