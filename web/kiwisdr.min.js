...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Sun Nov 16 09:19:44 2025) */
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Tue Nov 11 13:11:31 2025) */
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Tue Nov 11 12:55:28 2025) */
...
function w3_disable(el_id,disable,prop){if(!isArg(disable))disable=true;if(!isString(prop))prop='w3-disabled';el=w3_el(el_id);if(!el)return null;w3_set_props(el,prop,disable);if(isDefined(el.nodeName)&&(el.nodeName=='SELECT'||el.nodeName=='INPUT')){try{if(disable)
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Sun Nov 16 08:02:58 2025) */
...
function dx_click(ev,gid){var hold=(ev.type=='hold');var open_ext=(ev.type=='open_ext');var label;if(!hold&&!open_ext&&ev.shiftKey&&dx.db==dx.DB_STORED){dx_show_edit_panel(ev,gid);}else{if(!open_ext){w3_menu_close('dx_click');dx_sig_bw(gid);if(ev.target&&ev.target.nodeName=='A'){console.log('dx_click: link within label');dx.ctrl_click=false;return ev;}
...
var check_ext;if(kiwi_isMobile()){check_ext=open_ext;}else{check_ext=open_ext;}
