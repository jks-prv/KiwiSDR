...
/* web/extensions/ext.min.js (web/extensions/ext.js = Sun May 11 10:12:57 2025) */
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Wed May 14 12:09:40 2025) */
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Sun May 11 16:18:37 2025) */
...
w3_input_change(path,cb,'kd1');if(trace){console.log('w3_input_change '+path+'(2)...');console.log('el.value='+JSON.stringify(el.value));console.log(el);}}}else
if(el.value==''){if(trace)console.log('w3int_input_keydown: empty line + Enter');w3_input_change(path,cb,'kd2');w3_dismiss_keyboard(el);}}
if(ev.key=='Backspace'&&input_any_change&&el.selectionStart==0&&el.selectionEnd==el.value.length){if(trace)console.log('w3int_input_keydown Delete: len='+el.value.length+' ss='+el.selectionStart+' se='+el.selectionEnd);el.value='';w3_input_change(path,cb,'kd3');w3_dismiss_keyboard(el);}
...
function w3int_input_keyup(ev,path,cb){var k=ev.key.toUpperCase();var ctrl=ev.ctrlKey;var el=w3_el(path);if(!el)return;var disabled=w3_parent_with(el,'w3-disabled');var trace=w3_contains(el,'w3-trace');if(trace)console.log('w3int_input_keyup CE k='+k+(ctrl?' CTRL ':'')+' path='+path+' val=<'+el.value+'> cb='+cb+' disabled='+disabled);ev.stopPropagation();}
...
function w3_input_change(path,cb,from){w3int_input_change(null,path,cb,from);}
function w3int_input_change(ev,path,cb,from){var val='???';var retain=false;var trace=false;var el=w3_el(path);if(el){trace=w3_contains(el,'w3-trace');if(trace)console.log('w3int_input_change path='+path+' from='+from);if(cb){var cb_a=cb.split('|');if(isArg(from))cb_a.push(from);retain=w3_call(cb_a[0],path,el.value,false,cb_a);if(from=='in')return;}
w3_check_restart_reboot(el);if(trace)kiwi_trace();w3_schedule_highlight(el);val=el.value;}
w3_dismiss_keyboard(el);if(trace)console.log('w3int_input_change path='+path+' val='+val+' retain='+retain+' ev='+ev);if(retain===true){w3_field_select(el,{mobile:1});}else{w3int_post_action();}
...
if(kiwi_isMobile()){if(mobile_laptop_test)console.log('w3_menu_items MOBILE '+id);w3_iterate_children(id,function(el,i){el.addEventListener('touchmove',w3int_menu_touch_move,w3.BUBBLING);if(mobile_laptop_test)
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Wed May 14 14:30:04 2025) */
...
s='tmobile';if(q[s])mobile_laptop_test=true;s='ae';if(q[s])show_activeElement=true;s='fnas';if(q[s])force_need_autoscale=true;s='v';if(q[s]){debug_v=q[s];debug_v_set=true;console.log('URL: debug_v = '+debug_v);}
...
snd_send("SERVER DE CLIENT openwebrx.js SND");snd_send("SET dbug_v="+debug_v+','+(debug_v_set?1:0)+','+debug_v2+','+(debug_v2_set?1:0)+','+(dbgUs?1:0));w3_do_when_cond(function(){return(isArg(rx_chan)&&owrx.cfg_loaded);},function(){if(rx_chan!=0)return;var attn_ampl=0;if(gen_attn!=0){var dB=gen_attn+wf.cal;var ampl_gain=Math.pow(10,-dB/20);attn_ampl=0x01ffff*ampl_gain;}
...
function confirmation_panel_init2(){confirmation_panel_set_close_func(confirmation_panel_close);w3_el('id-kiwi-body').addEventListener('keyup',function(evt){if(evt.key=='Escape'){confirmation.close_cb();toggle_or_set_hide_panels(0);}},w3.CAPTURING);}
