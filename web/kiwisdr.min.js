...
/* web/extensions/ext.min.js (web/extensions/ext.js = Thu Feb 20 15:36:18 2025) */
...
var ext_zoom={TO_BAND:0,IN:1,OUT:-1,ABS:2,WHEEL:3,CUR:4,NOM_IN:8,MAX_IN:9,MAX_OUT:-9,TO_PASSBAND:null};function ext_tune(freq_dial_kHz,mode,zoom,zlevel,low_cut,high_cut,opt){var pb_specified=(isArg(low_cut)&&isArg(high_cut));extint.ext_is_tuning=true;freq_dial_kHz=freq_dial_kHz||(freq_displayed_Hz / 1000);freqmode_set_dsp_kHz(freq_dial_kHz,mode,opt);if(pb_specified)ext_set_passband(low_cut,high_cut);if(isArg(zoom)){zoom_step(zoom,Math.round(+zlevel));}else{zoom_step(ext_zoom.TO_BAND);}
...
function extint_select_build_menu(){var iframe_names=kiwi_array_iter(extint.iframe_names,function(el){return el.toLowerCase();});console.log(iframe_names);var iframe_enable=ext_get_cfg_param('iframe.enable');var s='';if(extint.ext_names&&isArray(extint.ext_names)){extint_enum_names(function(i,value,id,id_en){var enable=iframe_names.includes(id_en)?iframe_enable:ext_get_cfg_param(id_en+'.enable');console.log('extint_select_menu id_en='+id_en+' en='+enable);if(enable==null||ext_auth()==kiwi.AUTH_LOCAL)enable=true;if(id=='DRM')kiwi.DRM_enable=enable;if(id=='NAVTEX')id='NAVTEX/DSC';else
...
/* web/kiwi/kiwi.min.js (web/kiwi/kiwi.js = Wed Apr  2 11:41:43 2025) */
...
/* web/kiwi/kiwi_ui.min.js (web/kiwi/kiwi_ui.js = Wed Mar 19 08:47:28 2025) */
...
/* web/kiwi/kiwi_util.min.js (web/kiwi/kiwi_util.js = Sun Mar 16 13:50:25 2025) */
...
function kiwi_array_iter(a,func){var ra=[];a.forEach(function(el,i){ra[i]=func(el,i);});return ra;}
...
/* web/kiwi/w3_util.min.js (web/kiwi/w3_util.js = Tue Mar 11 11:24:59 2025) */
...
/* web/kiwi/ant_switch.min.js (web/kiwi/ant_switch.js = Wed Apr  2 11:17:36 2025) */
...
function ant_switch_config_focus(){ant_sw.focus_interval=setInterval(function(){ext_send('ADM antsw_GetCurrentAnt');},2000);}
...
return true;case"antsw_nch":console.log('antsw_nch='+param[1]);ant_switch_config_html2(+param[1]);return true;case"antsw_mix":ant_switch_log('antsw_mix='+param[1]);ant_sw.can_mix=(param[1]=='mix')?true:false;w3_innerHTML('id-antsw-mix','Supports mixing? '+(ant_sw.can_mix?'yes':'no'));w3_disable('id-antsw-mix-switch',!ant_sw.can_mix);return true;case"antsw_ip_or_url":ant_switch_log('antsw_ip_or_url='+param[1]);var ip_url=param[1];if(ip_url=='(null)')ip_url='';w3_set_value('id-ant_sw.ip_or_url',ip_url);ant_sw.ip_or_url=ip_url;return true;case"antsw_current_ant":console.log('antsw_current_ant: '+param[1]);var gnd=(param[1]=='g');if(gnd){w3_innerHTML('id-antsw-cur0','antenna(s) grounded');}
w3_hide2('id-antsw-cur0',!gnd);var a=param[1].split(',');for(i=1;i<=ant_sw.n_ant;i++){var sel=false;for(j=0;j<=a.length;j++){if(i==+a[j])sel=true;}
...
/* web/openwebrx/openwebrx.min.js (web/openwebrx/openwebrx.js = Fri Mar 28 09:04:10 2025) */
